...
This package exports some enums which are internally implemented as const objects and then the type is also exported as string literal unions. Constants are also present in the class as readonly members, mainly for backwards compatibility with kiteconnectjs. So in total there are 3 ways you can these, pick one that works for you!
